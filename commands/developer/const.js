'use strict';
const { runDeveloperCode } = require('../../utils/developerRuntime');
module.exports = {
    name: 'const', aliases: ['let', 'setvar'], description: 'Store a value in the developer JavaScript session', usage: '.const name = expression', category: 'developer',
    async execute({ args, sock, msg, m, from, sender, phoneNumber, reply, database, isOwner, isMod }) {
        if (!isOwner && !isMod) return reply('🔒 Developer tools are restricted to the bot owner and authorized moderators.');
        const result = await runDeveloperCode({ mode: 'const', code: args.join(' '), sock, msg, m, from, sender, phoneNumber, reply, database });
        return reply(result.ok ? `✅ *DEV CONST*\n\n${result.output || 'undefined'}` : `❌ *DEV ERROR*\n\n${result.error}`, { raw: true });
    },
};
