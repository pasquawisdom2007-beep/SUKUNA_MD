...
// up with the tightened text search only if the entity route came up short
// (or didn't resolve to a Wikidata item at all, e.g. non-person queries).
async function findImages(query) {
    const entityResults = await findEntityImages(query, MAX_RESULTS);
    if (entityResults.length >= MAX_RESULTS) return entityResults;

    const textResults = await searchCommonsPrecise(query);
    const seen = new Set(entityResults.map(item => item.url));
...
module.exports = {
    ...
    _private: {
        searchCommons, searchCommonsPrecise, findEntityImages, findImages,
