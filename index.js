...
async function main() {
    ...
    // Apply settings generated by the pair-site wizard before this session
    // receives commands. Existing users can still change them with commands.
    if (pairNumber) {
        const workMode = String(process.env.WORK_MODE || '').toLowerCase();
        if (workMode === 'public' || workMode === 'private') {
            database.setSelfMode(pairNumber, workMode === 'private');
        }
        const menuDesign = String(process.env.MENU_DESIGN || '').toLowerCase();
        if (menuDesign) database.setMenuDesign(pairNumber, menuDesign);
    }
