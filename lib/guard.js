...
async function warnMember(sock, entry, reason) {
    await sock.sendMessage(entry.groupId, {
        text: `${reason}\n\n@${numberPart(entry.userJid)} remains in the group because AntiBot is in warn-only mode.`,
        mentions: [entry.userJid],
    }).catch(() => {});
}

async function enforceFailedVerification(sock, entry, reason) {
    if (entry?.enforcementMode === 'warn') return warnMember(sock, entry, reason);
    return removeMember(sock, entry, reason);
}

async function expireEntry(sock, entry) {
    if (!entry || !pendingByPoll.has(entry.pollId)) return;
    removePending(entry);
    await enforceFailedVerification(sock, entry, '🚫 *Guard timeout:* the verification was not completed within 60 seconds.');
}
...
async function issueChallenge(sock, groupId, userJid, config) {
    ...
    const entry = {
        ...
        antiBot: config.antibot === true,
        enforcementMode: config.antibotMode === 'warn' ? 'warn' : 'kick',
    };
...
async function handleParticipantsUpdate(sock, event) {
    ...
    if (!config.guard && !config.antibot) return;

    const meta = await groupMetadata(sock, event.id);
    if (!botIsAdmin(meta, sock)) {
        await sock.sendMessage(event.id, {
            text: `⚠️ *${config.antibot && !config.guard ? 'AntiBot verification' : 'Guard'} is enabled,* but I need group-admin rights to verify and remove newcomers.`,
        }).catch(() => {});
        return;
    }
...
async function processVote(sock, entry, message, update, vote, alreadyDecrypted = false, route = 'messages.upsert') {
    ...
    if (outcome === 'correct') {
        ...
        await enforceFailedVerification(sock, entry, '🚫 *Guard failed:* the selected answer was incorrect.');
    }
}

function isPendingMember(groupId, userJid) {
    return pendingByUser.has(`${groupId}:${normalizeJid(userJid)}`);
}
...
async function processTextReply(sock, entry, message, text) {
    ...
    if (outcome === 'correct') {
        ...
        await enforceFailedVerification(sock, entry, '🚫 *Guard failed:* the reply was incorrect.');
    }
}
