...
function selectedHashes(vote) {
    return Array.isArray(vote?.selectedOptions)
        ? vote.selectedOptions.map(value => Buffer.from(value))
        : [];
}
...
async function expireEntry(sock, entry) {
    ...
    removePending(entry);
    await removeMember(sock, entry, '🚫 *Guard timeout:* the verification was not completed within 2 minutes.');
}
...
async function issueChallenge(sock, groupId, userJid, config) {
    ...
    const correctIndex = config.guardCorrect === 1 ? 1 : 0;
    ...
    try {
        await sock.sendMessage(groupId, {
            text: `🛡️ ${mention} — please answer the verification poll below within 2 minutes.`,
            mentions: [userJid],
        });
    ...
    const messageSecret = sent?.message?.messageContextInfo?.messageSecret
        || sent?.messageContextInfo?.messageSecret;
    const entry = {
        groupId,
        userJid,
        pollId,
        pollCreatorJid: normalizeJid(sock.user?.id),
...
function extractVoter(message) {
    return message?.key?.participant || message?.key?.participantAlt || message?.key?.remoteJid;
}

async function decryptSelectedOptions(sock, message, entry, vote) {
    ...
    try {
        const decoded = decryptPollVote(vote, {
            pollCreatorJid: entry.pollCreatorJid,
            pollMsgId: entry.pollId,
            pollEncKey: entry.pollEncKey,
            voterJid: normalizeJid(extractVoter(message)),
        });
        return selectedHashes(decoded);
    } catch (error) {
        console.error('[GUARD] poll vote decryption failed:', error.message);
        return [];
    }
}
...
async function handleMessages(sock, batch) {
    ...
    for (const message of batch?.messages || []) {
        ...
        const voter = extractVoter(message);
        if (!sameJid(voter, entry.userJid)) continue;

        const hashes = await decryptSelectedOptions(sock, message, entry, update.vote);
        removePending(entry);
        const correct = hashes.length === 1 && hashes[0].equals(entry.correctHash);
        if (correct) {
            await sock.sendMessage(entry.groupId, {
                text: `✅ *Guard passed:* @${numberPart(entry.userJid)} has been verified.`,
                mentions: [entry.userJid],
            }).catch(() => {});
        } else {
