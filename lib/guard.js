...
function selectedOptionIsCorrect(vote, decryptedHashes, entry) {
    const selected = selectedValues(vote);
    if (selected.length === 1) {
        const value = selected[0];
        const bytes = valueBuffer(value);
        if (bytes?.equals(entry.correctHash)) return true;
        if (typeof value === 'string' && normalizedOption(value) === normalizedOption(entry.correctOption)) return true;
    }
    return decryptedHashes.length === 1 && decryptedHashes[0].equals(entry.correctHash);
}
...
async function handleMessages(sock, batch) {
    ...
    for (const message of batch?.messages || []) {
        ...
        removePending(entry);
        const correct = selectedOptionIsCorrect(update.vote, hashes, entry);
        if (correct) {
            await sock.sendMessage(entry.groupId, {
