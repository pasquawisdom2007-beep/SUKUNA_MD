...
async function decryptSelectedOptions(sock, message, entry, vote, voterJid) {
    ...
    try {
        const decoded = decryptPollVote(vote, {
            ...
            voterJid: normalizeJid(voterJid),
        });
...
async function handleMessages(sock, batch) {
    ...
    for (const message of batch?.messages || []) {
        ...
        const hashes = await decryptSelectedOptions(sock, message, entry, update.vote, voter);
        removePending(entry);
