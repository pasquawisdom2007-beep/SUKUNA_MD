...
const { setupGuard, isPendingMember } = require('./guard');
...
class SessionManager {
    ...
    async startSession(phoneNumber, requestPairing = true) {
        ...
        try {
            ...
            this.sessions.set(phoneNumber, { sock, status: 'connecting', phoneNumber, antiBan });
            ...
            // ── AntiBot — verify newcomers and enforce only high-confidence signatures ──
            // A device-qualified JID alone is not proof of automation: legitimate
            // WhatsApp linked devices can expose the same shape. AntiBot therefore
            // lets Guard challenge every non-admin newcomer, while immediate removal
            // is reserved for explicit bot metadata or a known bot-library stamp.
            sock.ev.on('group-participants.update', async (u) => {
                try {
                    ...
                    const mode = grp.antibotMode === 'warn' ? 'warn' : 'kick';
                    ...
                    const botParticipant = meta.participants?.find(participant =>
                        participantIdentifiers(participant).some(id => botIdentities.some(bot => sameIdentity(id, bot)))
                    );
                    const botIsAdmin = !!botParticipant?.admin;

                    for (const participantId of (u.participants || [])) {
                        if (botIdentities.some(bot => sameIdentity(participantId, bot))) continue;
                        const participant = findParticipant(meta, participantId);
                        if (participant?.admin) continue;

                        const detection = detectBotSignals({ jid: participantId, participant });
                        if (!detection.highConfidence) {
                            // Weak linked-device candidates are handled by Guard's
                            // sender-bound challenge; do not falsely kick them here.
                            if (detection.candidate) {
                                console.log(`[ANTIBOT] weak newcomer candidate @${shortJid(participantId)} queued for verification`);
                            }
                            continue;
                        }

                        const shortNum = shortJid(participantId);
                        const reason = detection.reason || 'high-confidence bot signature';
                        if (mode === 'kick' && botIsAdmin) {
                            try {
                                await sock.groupParticipantsUpdate(u.id, [participantId], 'remove');
                                await sock.sendMessage(u.id, {
                                    text: `🤖 *AntiBot:* @${shortNum} was removed — ${reason}.`,
                            ...
                            await sock.sendMessage(u.id, {
                                text: `⚠️ *AntiBot Warning:* @${shortNum} matched ${reason}.${
                                    mode === 'kick' && !botIsAdmin
                                        ? '\n\nMake me a group admin to enable auto-removal.'
                                        : ''
                                }`,
                                mentions: [participantId],
                            }).catch(() => {});
    ...
    async handleMessages(sock, phoneNumber, { messages, type }) {
        ...
        for (const msg of messages) {
            try {
                ...
                // ── AntiBot enforcement ──────────────────────────────────────
                // A linked-device JID is only a weak candidate because legitimate
                // WhatsApp companion devices can use that shape. Auto-removal is
                // reserved for explicit bot metadata or a known bot-library stamp;
                // all other newcomers are protected by the Guard challenge.
                if (!fromMe && isGroup) {
                    try {
                        ...
                        if (grp.antibot) {
                            ...
                            const senderShort = shortJid(sender);
                            const senderIsGroupAdmin = participant?.admin === 'admin' || participant?.admin === 'superadmin';
                            if (detection.highConfidence && !senderIsGroupAdmin) {
                                const mode = grp.antibotMode === 'warn' ? 'warn' : 'kick';
                            ...
                            if (mode === 'kick' && botIsAdmin) {
                                ...
                                await sock.sendMessage(from, {
                                    text: `🤖 *AntiBot:* @${senderShort} was removed — ${reason}.`,
                                    mentions: [sender],
                                }).catch(() => {});
                            } else {
                                await sock.sendMessage(from, {
                                    text: `⚠️ *AntiBot:* @${senderShort} matched ${reason}.${
                                        mode === 'kick' && !botIsAdmin
                                            ? '\n\nMake me a group admin to enable auto-removal.'
                                            : ''
                                    }`,
                ...
                // Newcomers under Guard/AntiBot are quarantined from commands and
                // moderation until their sender-bound verification succeeds. This
                // check comes after high-confidence AntiBot signatures so a known
                // bot-library stamp is never hidden by the quarantine.
                if (isGroup && !fromMe && !senderIsOwner) {
                    try {
                        if (isPendingMember(from, sender)) continue;
                    } catch (_) {}
                }
