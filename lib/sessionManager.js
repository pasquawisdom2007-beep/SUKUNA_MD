        ...
        try {
            ...
            sock.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
                ...
                if (connection === 'close') {
                    ...
                    if (sc?.pairingPending) {
                        sc.status = 'pairing_failed';
                        console.error(`[PAIR] Pairing socket closed before a usable code was returned for ${phoneNumber}.`);
                        return;
                    }
                    // If WhatsApp asks for a restart after the user submits the
                    // code, keep the session alive. If registration has not yet
                    // completed, the reconnect must request a fresh code rather
                    // than silently opening an unregistered socket.
                    const shouldPairAgain = !!sc?.pairingCodeIssued && !state.creds.registered;
