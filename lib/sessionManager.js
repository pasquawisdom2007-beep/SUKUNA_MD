    ...
    async handleMessages(sock, phoneNumber, { messages, type }) {
        ...
        for (const msg of messages) {
            try {
                ...
                // ── Protected developer JavaScript ─────────────────────────
                // The owner/mod developer surface intentionally runs before
                // normal command parsing, so `$ ...`, `const ...`, `eval ...`,
                // and `sock...` do not need the bot prefix. Ordinary users can
                // never reach this evaluator, even if their text resembles JS.
                const developerInput = body.trim();
                const developerMode = /^const\s+[A-Za-z_$][\w$]*\s*=/.test(developerInput)
                    ? 'const'
                    : 'eval';
                const isDeveloperCode = /^\$\s*[\s\S]+/i.test(developerInput) ||
                    /^(?:const\s+[A-Za-z_$][\w$]*\s*=|eval\s+|sock(?:\.|\[)|await\s+sock\.)/i.test(developerInput);
                if (isDeveloperCode) {
                    ...
                    if (developerAllowed) {
                        try {
                            ...
                            const code = developerInput.replace(/^\$\s*/, '').replace(/^eval\s+/i, '');
                            const evaluated = await runDeveloperCode({
                                mode: developerMode, code, sock, msg, m: developerM,
                                from, sender, phoneNumber, reply, database,
                            });
                            await reply(evaluated.ok ? `✅ *DEV RESULT*\n\n${evaluated.output || 'undefined'}` : `❌ *DEV ERROR*\n\n${evaluated.error}`, { raw: true });
                        } catch (error) {
                            await reply(`Developer runtime error: ${error.message}`, { raw: true });
                        }
                    }
                    continue;
                }
                ...
                // ── Owner-only guard ──────────────────────────────────────────
                const senderIsMod = this._isModUser(phoneNumber, sender);
                if ((command.category === 'owner' || command.category === 'developer') && !senderIsOwner && !senderIsMod) {
                    await reply(command.category === 'developer'
                        ? '🔒 *Developer tools are restricted to the bot owner and authorized moderators.*'
