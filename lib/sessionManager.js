    ...
    async handleMessages(sock, phoneNumber, { messages, type }) {
        ...
        for (const msg of messages) {
            try {
                ...
                // Track per-group activity for .kick inactive and the
                // .listactive / .listinactive commands (last-seen + msg count).
                if (isGroup && !fromMe) {
                    try { database.markSeen(from, sender); } catch (_) {}
                    try { database.incMessageCount(from, sender); } catch (_) {}
                }
