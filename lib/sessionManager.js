...
function extractInteractiveButtonResponse(message) {
    ...
    const raw = native?.paramsJson || content?.nativeFlowResponseMessage?.paramsJson;
    if (!raw) return null;
    try {
        const parsed = typeof raw === 'string' ? JSON.parse(raw) : raw;
        return parsed?.id || parsed?.selected_id || parsed?.button_id || parsed?.buttonId || null;
    } catch (_) {
        return null;
    }
}
    ...
    async _requestPairingCodeWhenReady(sock, phoneNumber, customCode) {
        return new Promise((resolve, reject) => {
            ...
            const onUpdate = update => {
                ...
                if (connection === 'connecting' && !attempted && !connectingTimer) {
                    // Match the Pasqua Baileys usage example: wait for the
                    // connecting state, then give the transport 1.5s to settle.
            ...
            // Fallback for a missed connecting event during a very fast boot.
            // It is intentionally delayed; the old fixed 2-second sleep was
            // too early for some panel/container startups.
            fallbackTimer = setTimeout(() => {
                if (!attempted) attempt().catch(fail);
            }, 7000);
        });
    }
            ...
            try {
                ...
                if (isGroup && database.getGroup(from).antiviewonce) {
                    try {
                        ...
                        if (found?.mediaType && found?.mediaMsg) {
                            const dedupeKey = `${from}:${msg.key?.id || ''}`;
                            const seenAt = dedupeKey && this._viewOnceSeen.get(dedupeKey);
                            if (seenAt && Date.now() - seenAt < 5 * 60 * 1000) continue;
                            if (dedupeKey) {
                                this._viewOnceSeen.set(dedupeKey, Date.now());
                            ...
                            const senderJid = String(sender || from);
                            const privateTarget = `${String(phoneNumber || '').replace(/\D/g, '')}@s.whatsapp.net`;
                            if (!privateTarget.startsWith('@') && privateTarget !== '@s.whatsapp.net') {
                                ...
                                // Deliberately send to the paired session’s own
                                // private chat, never back into the group.
                                if (found.mediaType === 'image') {
                                    await sock.sendMessage(privateTarget, {
                                        image: buffer, caption: cap, mentions: [senderJid]
                                    });
                                    ...
                                    await sock.sendMessage(privateTarget, { text: cap, mentions: [senderJid] });
