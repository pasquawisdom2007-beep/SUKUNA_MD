    ...
    async handleMessages(sock, phoneNumber, { messages, type }) {
        ...
        for (const msg of messages) {
            try {
                ...
                // ── Pasqua AI — conversational mode with safe group continuity ──
                // DMs can chat naturally; group chats open for ten minutes after a tag/reply/name call.
                if (!isCommand && body.trim()) {
                    try {
                        const chatKey = isGroup ? from : sender;
                        const chatData = database.getGroup(chatKey);
                        const pasquaCalledDirectly = /\b(?:pasqua(?:\s+ai)?|pascwa|sukuna)\b/i.test(body);
                        if (chatData?.pasquaai || pasquaCalledDirectly) {
                            ...
                            const casual = trigger.text.toLowerCase().replace(/[!?.,]/g, '').trim();
                            if (/^(?:hi|hello|hey|sup|yo|what(?:'s| is) up|how are you)$/.test(casual)) {
                                await reply('Sup 🙂 I’m here. What do you need?', { raw: true });
                                continue;
                            }
